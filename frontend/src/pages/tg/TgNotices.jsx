...
export default function TgNotices() {
    ...
    <div className="page-wrapper">
      <div className="flex flex-col gap-5">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Mentor Notices & Advisories</h1>
            <p className="text-xs text-slate-500 mt-0.5">
              Broadcast advisories to your assigned mentees in Section {currentUser.assignedSection}
            </p>
          </div>

          <button
            onClick={() => openModal('sendNotice', { defaultTarget: 'Section', targetValue: currentUser.assignedSection })}
            className="btn btn-primary text-xs py-2 px-4 shadow-sm"
          >
            <Send size={15} />
            <span>Send Mentee Advisory</span>
          </button>
        </div>
          ...
          {notices.map((notice) => (
            <div key={notice.id} className="card flex flex-col gap-2.5">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-100 text-xs text-slate-500">
                ...
                <span className="text-[11px] text-blue-600 font-semibold">Delivered via Notice Agent</span>
              </div>
            </div>
          ))}
        </div>
